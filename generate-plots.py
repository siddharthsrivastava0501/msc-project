...
if __name__ == "__main__":
    ...
    gt_config = {
        ...
        'nr': nr,
        'C': C,
        'a': torch.empty((nr,)).uniform_(3, 6),
        'b': torch.empty((nr,)).uniform_(3, 6),
        'c': torch.empty((nr,)).uniform_(3, 6),
        'd': torch.empty((nr,)).uniform_(3, 6),
        'P': torch.empty((nr,)).uniform_(0.4, 0.8),
        'Q': torch.empty((nr,)).uniform_(0.4, 0.8),
        'dyn_noise': 0.2,
        'obs_noise': 0.02,
    }
    ...
    with open(f'report_results_abcdPQ_dualhybrid_3nr_10iters_0.2dyn0.02obs_C0/abcd_{iters}iters_{schedule}_run{run_id}.pkl', 'wb') as f:
        pickle.dump(res, f)
